...
public class CardGenerator {
    ...
    public static List<olof.sjoholm.game.logic.BoardAction> generateList(int nrOfCards) {
        ...
        List<olof.sjoholm.game.logic.BoardAction> list = new ArrayList<olof.sjoholm.game.logic.BoardAction>(nrOfCards);
        for (int i = 0; i < nrOfCards; i++) {
            olof.sjoholm.game.logic.BoardAction boardAction = factories.get().getRandom();
            boardAction.setId(i);
    ...
    private static class RotateFactory implements BoardActionFactory {
        ...
        @Override
        public olof.sjoholm.game.logic.BoardAction getRandom() {
            Rotation rotation = rotationGenerator.get();
            return new olof.sjoholm.game.logic.BoardAction.Rotate(rotation);
        }
    }
    ...
    private static class MovementFactory implements BoardActionFactory {
        ...
        public olof.sjoholm.game.logic.BoardAction getRandom() {
            ...
            return new olof.sjoholm.game.logic.BoardAction.MoveForward(randSteps);
        }
    }

    private interface BoardActionFactory {

        olof.sjoholm.game.logic.BoardAction getRandom();
    }

    private static class RandomGenerator<T> {
        List<olof.sjoholm.utils.Pair<Integer, T>> objects = new ArrayList<olof.sjoholm.utils.Pair<Integer, T>>();
        int size;
        ...
        public void add(int weight, T object) {
            ...
            objects.add(new olof.sjoholm.utils.Pair<Integer, T>(size, object));
            size += weight;
        }
        ...
        public T get() {
            ...
            for (int i = objects.size() - 1; i >= 0; i--) {
                olof.sjoholm.utils.Pair<Integer, T> pair = objects.get(i);
                if (rand > pair.key) {
                    return pair.value;
                }
            }
